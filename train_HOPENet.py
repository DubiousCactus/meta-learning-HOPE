...
class BaseTrainer(ABC):
    ...
    def __init__(self, model_name: str, dataset_root, lr, lr_step, lr_step_gamma, batch_size,
            use_cuda=False, gpu_number=0, test=False):
        ...
        if not test:
            self.train_data_loader, self.val_data_loader = self._load_train_val(dataset_root,
                    batch_size)
        else:
            self.test_data_loader = self._load_test(dataset_root, batch_size)
...
class MAMLTrainer(BaseTrainer):
    def __init__(self, model_name: str, dataset_root, lr, lr_step, lr_step_gamma, batch_size,
            use_cuda=False, gpu_number=0, test=False):
        super().__init__(model_name, dataset_root, lr, lr_step, lr_step_gamma, batch_size,
                use_cuda=use_cuda, gpu_number=gpu_number, test=test)
...
class HOPETrainer(MAMLTrainer):
    ...
    def _training_step(self, batch: tuple, learner, steps: int, shots: int):
        ...
        for step in range(steps):
            ...
            support_loss = ((self.lambda_1)*loss2d_init + (self.lambda_1)*loss2d +
                    (self.lambda_2)*loss3d)
        ...
        query_loss = ((self.lambda_1)*e_loss2d_init + (self.lambda_1)*e_loss2d +
                (self.lambda_2)*e_loss3d)
...
def main(args):
    hope_trainer = HOPETrainer("hopenet", args.input_file, args.learning_rate, args.lr_step,
